...
This means that you can run Functions written in any supported language inside a TypeScript or JavaScript application ([Host Support](#host-support)) _as well as_ **write** functions in JS/TS and run them in other host languages, like [Golang](/scale/languages/golang/overview).
...
<Tabs items={['Embedding the Function']}>
    <Tab value="Embedding the Function">
        ```js
        import { New, Config } from "@loopholelabs/scale";
        import { Read } from "@loopholelabs/scale/scalefunc/index.js";
...
adapters for popular frameworks and libraries, such as [NextJS](/scale/languages/javascript-typescript/adapters/nextjs).
