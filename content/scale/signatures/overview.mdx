...
We use the [Polyglot](/polyglot) library under the hood to facilitate efficient
...
|--------------------------------|-------------------------------------------------------------------|----------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|
| [HTTP](/scale/signatures/http) | HTTP Signature meant to be used for HTTP handlers and middleware. | Go, Rust, TypeScript, JavaScript | [net/http](/scale/languages/golang/adapters/net-http), [FastHTTP](/scale/languages/golang/adapters/fasthttp), [NextJS](/scale/languages/javascript-typescript/adapters/nextjs) | 0.3.8          |

Official are maintained by the Scale Functions team and come with [Adapters](/scale/languages/golang/adapters/net-http) which
...
For more information on using this syntax, using [HCL format](/scale/signatures/hcl)

## Using a Signature

Generate the signature, defined in the example above, by again using [the CLI](/scale/cli/reference/signatures):

```
scale signature generate <name:tag>
```

This will generate both the `guest` and `host` signatures, each of which are set of functions/interfaces that handle getting the data in, and out, of the associated scale function(s), in a type-safe, and performant manner.
For more information on why we need bindings for the guest and the host, see the [overview](/scale).
...
All of the commands for working with signatures are in the [CLI API reference](/scale/cli/reference/signatures).

## Next Steps

Now that you know the basics of Scale Signatures in Golang, you can start integrating them into your Scale plugins/functions. For more info, please refer to the [API Reference](/scale/cli/reference/signatures).
